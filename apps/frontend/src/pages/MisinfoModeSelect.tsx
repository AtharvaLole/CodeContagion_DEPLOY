...
function createShopFrontTexture(width: number, height: number) {
  const canvas = document.createElement("canvas");
  canvas.width = width;
  canvas.height = height;
  const ctx = canvas.getContext("2d");
  ...
  for (let i = 0; i < 20; i++) {
    ...
    ctx.strokeStyle = i % 2 === 0 ? "rgba(34,211,238,0.20)" : "rgba(244,114,182,0.14)";
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(20, y);
  ...
  ctx.fillStyle = "#f8fafc";
  ctx.font = `bold ${Math.floor(height * 0.24)}px Arial`;
  ctx.fillText("DEBUG CAFE", width / 2, height * 0.48);

  ctx.fillStyle = "#22d3ee";
  ctx.font = `bold ${Math.floor(height * 0.09)}px Arial`;
  ctx.fillText("CODECONTAGION", width / 2, height * 0.74);
...
function createVerticalCafeTexture(width: number, height: number) {
  const canvas = document.createElement("canvas");
  canvas.width = width;
  canvas.height = height;
  const ctx = canvas.getContext("2d");
  ...
  ctx.shadowBlur = 18;
  ctx.fillStyle = "#f8fafc";
  ctx.font = `bold ${Math.floor(width * 0.28)}px Arial`;

  const letters = ["C", "A", "F", "E"];
  const startY = height * 0.24;
  const gap = height * 0.16;
  letters.forEach((letter, i) => {
    ctx.fillText(letter, width / 2, startY + i * gap);
  });
...
function RamenScene() {
  ...
  const { scene } = useGLTF("/ramen-shop/models/ramenShop/glTF/ramenShop.gltf", "/draco/gltf/");
  ...
  const brandedTextures = useMemo(() => {
    return {
      ...
      shopFront: createShopFrontTexture(1600, 500),
      shopSide: createVerticalCafeTexture(500, 1400),
    };
  }, []);

  const preparedScene = useMemo(() => {
    [ramenPng, machinesPng, floorPng, miscPng].forEach(configureTexture);
    ...
    return cloned;
  }, [scene, ramenPng, machinesPng, floorPng, miscPng, matcaps, brandedTextures]);
  ...
  useFrame((state, delta) => {
    ...
    animateScreen("arcadeScreen");
    animateScreen("vendingMachineScreen");
  ...
  return (
    <group ref={groupRef} position={[0, -2.9, 0]} rotation={[0.01, -0.58, 0]} scale={1.22}>
      ...
      {/* front sign overlay */}
      <mesh
        position={[-4.72, 1.02, 3.42]}
        rotation={[0, 0.98, 0]}
        renderOrder={20}
      >
        <planeGeometry args={[3.62, 1.12]} />
        <meshBasicMaterial
          map={brandedTextures.shopFront}
          toneMapped={false}
          polygonOffset
      ...
      <mesh
        position={[-7.08, 2.06, 1.58]}
        rotation={[0, 2.55, 0]}
        renderOrder={20}
      >
        <planeGeometry args={[1.08, 2.72]} />
        <meshBasicMaterial
          map={brandedTextures.shopSide}
          toneMapped={false}
          polygonOffset
          polygonOffsetFactor={-4}
        />
